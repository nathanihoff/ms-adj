...
The tables below provide estimates from the ASR replications presented in the main text, but with the addition of two other models. These implement methods proposed by @clarke_2024_double that incorporate individual or group fixed effects into DML. The authors' package, `XTDML`, builds on the `DoubleML` package. I use two of their procedures. First, correlated random effects (CRE) models explicitly models the correlation between the group-level components in both the treatment and outcome models. Second, the authors' "hybrid" model combines CRE with a within-group estimator that partials out group means from all variables, similar to the function of fixed effects in OLS.
...
readRDS(here('replication', 'biegert_rep.RDS')) %>%
  ...
  huxtable::set_top_border((nrow(.)-5):(nrow(.)-1), value = 0) %>%
  huxtable::set_top_border((nrow(.)-5), 2:7) %>%
