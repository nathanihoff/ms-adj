...
```{r load}
# data from mx0.5
pisa <- readRDS('/Users/nathan/My Drive/1 UCLA/Dissertation/diss-prop/data/pisa.rds') %>%
...
The 20 data sets used here cover a range of these attributes; see the supplemental material from @dorie_2019_automated for details. For these preliminary results, I used 5 simulations of each data set, resulting in 100 data sets. I then calculated the same statistics as in Table \@ref(tab:sim-results): percent bias, root-mean squared error, and median average error. I also include the number of data sets for which the method failed and the average computation time for each data set.
...
sim_linear <- list()
  for(j in 1:100){ # up to 100
    print(j)
    sim_linear[[j]] <- aciccomp2016::dgp_2016(aciccomp2016::input_2016, 
                                      3, j, extraInfo = T)
...
lm_sim <- function(dat){
  ...
  return(list(
    ...
    comp_time = as.numeric(Sys.time() - start_time)/(n-fail_count))
    )
}
...
lm_df <- perform(lm_sim_out$est_df, 'OLS') %>%
  mutate(fail_count = lm_sim_out$fail_count,
         comp_time = lm_sim_out$comp_time)


write_csv(lm_df, here('files', 'lm_df2.csv'))
```
...
psm_sim <- function(dat){
  ...
  for(i in 1:n){
    ...
    form <- as.formula(paste0('d ~ ', paste(names(sim_dat[3:60]), collapse = '+')))
    ...
    form2 <- as.formula(paste0('y ~ d + ', paste(names(sim_dat[3:60]), collapse = '+')))
    
  tryCatch({
    psm_out <- lm(form2, 
                  MatchIt::match.data(match_out), 
                  weights = weights)
    psm_list[[i]] <- data.frame(d = psm_out$coefficients[['d']],
                               truth = mean(dat[[i]]$y.1) - mean(dat[[i]]$y.0))
  ...
  return(list(
    ...
    fail_count = fail_count,
    comp_time = as.numeric(end_time - start_time)/(n-fail_count))
  )
}
...
psm_df <- perform(psm_sim_out$est_df, 'PSM') %>%
  mutate(fail_count = psm_sim_out$fail_count,
         comp_time = psm_sim_out$comp_time)

write_csv(psm_df, here('files', 'psm_df2.csv'))
```
...
aipw_sim <- function(dat, seed = 185){
  ...
  for(i in 1:n){
    ...
    # Need to filter out cases without overlap (CausalGAM does not)
    treat_levels <- sim_dat %>% 
      filter(d == 1) %>%
      select(where(is.character)) %>%
      sapply(unique)
    control_levels <- sim_dat %>% 
      filter(d == 0) %>%
      select(where(is.character)) %>%
      sapply(unique)
    
    for(j in 1:length(treat_levels)){
      if(!all(c(treat_levels[[j]] %in% control_levels[[j]]),control_levels[[j]] %in% treat_levels[[j]])){
        # print(names(treat_levels)[j])
        # print(treat_levels[[j]])
        # print(control_levels[[j]])

        var_name <- names(treat_levels)[j]

        sim_dat <- sim_dat %>%
          filter(!!sym(var_name) %in% c(treat_levels[[j]]) &
                   !!sym(var_name) %in% c(control_levels[[j]]))
    ...
    class_vec <- sapply(sim_dat, function(x) class(x))
    cont_names <- names(sim_dat[, class_vec == 'numeric'])
    cont_names <- cont_names[3:length(cont_names)]
    not_cont_names <- names(sim_dat[, class_vec != 'numeric'])
    
    pscore_form <- as.formula(paste0('d ~ s(', 
                                     paste(cont_names, collapse = ') + s('), ') +',
                                     paste(not_cont_names, collapse = ' + ')))
    outcome_form <- as.formula(paste0('y ~ s(', 
                                     paste(cont_names, collapse = ') + s('), ') +',
                                     paste(not_cont_names, collapse = ' + ')))
    
    
    tryCatch({
      aipw_out <- CausalGAM::estimate.ATE(
      pscore.formula = pscore_form,
      pscore.family = binomial,
      outcome.formula.t =outcome_form,
      outcome.formula.c = outcome_form,
      outcome.family = gaussian,
      treatment.var = 'd',
      data = sim_dat,
      divby0.action="t",
      ...
      aipw_list[[i]] <- data.frame(d = aipw_out$ATE.AIPW.hat,
                               truth = mean(dat[[i]]$y.1) - mean(dat[[i]]$y.0))
...
aipw_df <- perform(aipw_out$est_df, 'AIPW') %>%
  mutate(fail_count = aipw_out$fail_count,
         comp_time = aipw_out$comp_time)

write_csv(aipw_df,  here('files', 'aipw_df2.csv'))
...
tmle_sim <- function(dat){
  ...
  pscore_form <- as.formula(paste0('A ~ ', 
                                   paste(names(dat[[1]]$x), collapse = ' + ')))
  outcome_form <- as.formula(paste0('y ~ A +', 
                                   paste(names(dat[[1]]$x), collapse = ' + ')))
  ...
  for(i in 1:n){
   ...
   tryCatch({
    ...
    tmle_out <- tmle::tmle(
                Y = sim_dat$Y,
                A = sim_dat$A,
                W = select(sim_dat, x_1:x_58),
                Qform = Y ~ .,
                gform = A ~ .)
      
      tmle_list[[i]] <- data.frame(d = tmle_out$estimates$ATE$psi,
                               truth = mean(dat[[i]]$y.1) - mean(dat[[i]]$y.0))
...
tmle_df <- perform(tmle_out$est_df, 'TMLE') %>%
  mutate(fail_count = tmle_out$fail_count,
         comp_time = tmle_out$comp_time)

write_csv(tmle_df, here('files', 'tmle_df2.csv'))
```
...
dml_df <- perform(dml_out$est_df, 'DML') %>%
  mutate(fail_count = dml_out$fail_count,
         comp_time = dml_out$comp_time)

write_csv(dml_df, here('files', 'dml_df2.csv'))
```

```{r dorie-results}
bind_rows(
  read_csv(here('files', 'lm_df2.csv')),
  read_csv(here('files', 'psm_df2.csv')),
  read_csv(here('files', 'aipw_df2.csv')),
  read_csv(here('files', 'tmle_df2.csv')),
  read_csv(here('files', 'dml_df2.csv'))
  ) %>%
  select(-comp_time) %>%
  kableExtra::kable(booktabs = T, 
                    linesep = '',
...
bind_rows(
  read_csv(here('files', 'lm_df3.csv')),
  read_csv(here('files', 'psm_df3.csv')),
  read_csv(here('files', 'aipw_df3.csv')),
  read_csv(here('files', 'tmle_df3.csv')),
  read_csv(here('files', 'dml_df3.csv'))
  ) %>%
  select(-comp_time) %>%
  kableExtra::kable(booktabs = T, 
                    linesep = '',
...
```{r lm-pisa, eval = F}
lm_out_pisa <- lm(math1 ~ immigrant + mom_ed + dad_ed + 
                          female + early_ed + cultural_pos + home_ed + age, 
                   pisa_test) %>%
  tidy()


lm_df <- data.frame(method = 'OLS',
                     ATE = pull(lm_out_pisa, estimate)[lm_out_pisa$term == 'immigrant'],
                     se = pull(lm_out_pisa, std.error)[lm_out_pisa$term == 'immigrant'])
```


```{r psm-pisa, eval = F}
match_pisa <- MatchIt::matchit(immigrant ~ mom_ed + dad_ed + 
                          female + early_ed + cultural_pos + home_ed + age,
                       data = pisa_test,
                       method = 'nearest',
                       distance = 'glm') 

psm_out_pisa <- lm(math1 ~ immigrant + mom_ed + dad_ed + 
                          female + early_ed + cultural_pos + home_ed + age, 
                   MatchIt::match.data(match_pisa), 
                   weights = weights)

psm_df <- lmtest::coeftest(psm_out_pisa, 
                            vcov. = sandwich::vcovCL, 
                            cluster = ~subclass) %>%
  tidy()

psm_df <- data.frame(method = 'PSM',
                     ATE = pull(psm_df, estimate)[psm_df$term == 'immigrant'],
                     se = pull(psm_df, std.error)[psm_df$term == 'immigrant'])
```

```{r aipw-pisa, eval = F}
library(gam)

pisa_test_dummies <- pisa_test %>%
  select(-early_ed) %>%
  bind_cols(model.matrix(~ early_ed, data = pisa_test)) %>%
  as.data.frame()

aipw_pisa_out <- CausalGAM::estimate.ATE(
        pscore.formula = immigrant ~ s(mom_ed) + s(dad_ed) + female + 
          `early_ed0-1` + `early_ed1+` + s(cultural_pos) + s(home_ed) + s(age),
        pscore.family = binomial,
        outcome.formula.t = math1 ~ s(mom_ed) + s(dad_ed) + female + 
          `early_ed0-1` + `early_ed1+` + s(cultural_pos) + s(home_ed) + s(age),
        outcome.formula.c = math1 ~ s(mom_ed) + s(dad_ed) + female + 
          `early_ed0-1` + `early_ed1+` + s(cultural_pos) + s(home_ed) + s(age),
        outcome.family = gaussian,
        treatment.var = 'immigrant',
        data = pisa_test_dummies,
        divby0.action="t",
        divby0.tol=0.001,
        var.gam.plot=FALSE,
        nboot = 0)
...
aipw_df <- data.frame(method = 'AIPW',
                     ATE = aipw_pisa_out$ATE.AIPW.hat,
                     se = aipw_pisa_out$ATE.AIPW.asymp.SE)
```
...
tmle_df <- data.frame(method = 'TMLE',
           ATE = tmle_out$estimates$ATE$psi,
           se = sqrt(tmle_out$estimates$ATE$var.psi))
...
obj_dml_plr = DoubleML::DoubleMLPLR$new(dml_data, ml_l=ml_l, ml_m=ml_m)
obj_dml_plr$fit()




dml_df <- data.frame(method = 'DML',
           ATE = obj_dml_plr$all_coef,
           se = obj_dml_plr$se[['immigrant']])
```
...
```{r pisa-results}
# bind_rows(lm_df, psm_df, aipw_df, tmle_df, dml_df)  %>%
#   write_csv(here('files', 'pisa_df.csv'))

read_csv(here('files', 'pisa_df.csv')) %>%
  kableExtra::kable(booktabs = T, 
                    linesep = '',
